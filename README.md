...
```bash
cd backend
python -m pytest -q      # 120 tests (analytics + API security + rate-limit contract + resource bounds)
```
...
- Automated tests cover the analytics layer, the API security boundary, the rate-limit contract, resource bounds and the cold-start contract (120 backend tests) plus 22 frontend tests; ingestion and the simulation routers are not under test.
