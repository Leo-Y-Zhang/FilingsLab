...
- Automated tests cover the analytics layer, the API security boundary, the rate-limit contract, error disclosure, research integrity, resource bounds and the cold-start contract (146 backend tests) plus 22 frontend tests; ingestion is not under test, and the simulation and comparison routers are covered only on their failure path.
